...
/*
 * Prim's Minimum Spanning Tree algorithm, graph is represented by adjancency list. 
 * Uses min-heap
 */
...
prim(graph_t *G)
{
	...
	while(!minheap_empty(heap)) {
		...
		while(current != NULL) {
			v = current->dst;
			if(minheap_exists(heap, v) && current->weight < dist[v]) {
				dist[v] = current->weight;
